// VENDORED from adabraka-ui (MIT) — see crates/app/VENDORED.md for source rev
// and the complete list of local modifications.
//
// Clippy style lints are allowed here (with the list kept explicit) to keep
// the diff against upstream minimal — style-refactoring vendored code makes
// every future refresh a merge conflict. Correctness lints still apply.
#![allow(
...
pub struct InputState {
    ...
    // Vendored: driven by adabraka's shake animation, which we don't render.
    #[allow(dead_code)]
    pub(crate) shake_count: u32,
    cursor_position_override: Option<usize>,

    // Local modification: colors injected from the app Theme instead of
    // adabraka's use_theme() global (no hard-coded colors in crates/app).
...
impl InputState {
    pub fn new(cx: &mut Context<Self>) -> Self {
        Self {
            ...
            // Local modification: neutral defaults; callers apply the real
            // Theme via `with_colors`.
            placeholder_color: hsla(0.0, 0.0, 0.5, 1.0),
            cursor_color: hsla(210.0 / 360.0, 0.9, 0.55, 1.0),
            selection_color: hsla(210.0 / 360.0, 0.9, 0.55, 0.25),
        }
    }

    /// Local modification: inject colors from the app Theme.
    pub fn with_colors(mut self, placeholder: Hsla, cursor: Hsla, selection: Hsla) -> Self {
    ...
    pub fn set_value(
        ...
        // Local modification: set_value replaces the ENTIRE content. Upstream
        // delegated to replace_text_in_range(None, ..), which only replaces
        // the current selection/marked range — surprising append semantics.
    ...
    pub fn tab(&mut self, _: &Tab, window: &mut Window, cx: &mut Context<Self>) {
        window.focus_next(cx); // Local modification: gpui at our rev takes cx
        cx.emit(InputEvent::Tab);
    }

    pub fn shift_tab(&mut self, _: &ShiftTab, window: &mut Window, cx: &mut Context<Self>) {
        window.focus_prev(cx); // Local modification: gpui at our rev takes cx
...
impl gpui::Element for InputTextElement {
    ...
    fn prepaint(
        ...
        // Local modification: colors come from InputState (app Theme), not a
        // library-global theme.
        ...
        let (selection, cursor) = if selected_range.is_empty() {
            (
                ...
                Some(fill(
                    ...
                    cursor_color, // Local modification: theme-injected (was rgb(0x0066ff))
                )),
            )
        } else {
            (
                Some(fill(
                    ...
                    selection_color, // Local modification: theme-injected (was rgba(0x3311ff30))
                )),
                None,
            )
        };
    ...
    fn paint(
        ...
        if line
            // Local modification: gpui at our rev takes align + wrap width.
            .paint(
                bounds.origin,
                window.line_height(),
