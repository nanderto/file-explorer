...
**M7a complete in code** — the real theme system, on top of M1–M6.
`crates/theme` now exists (the repo layout has claimed it since M0): the
theme **model + JSON loader**, gpui-typed but I/O-free. `crates/app` gained
the glue.

* **`Theme` is a global, not a constructor argument.** Every view used to be
  handed a `Theme` and keep a clone, which cannot answer "the theme changed
  while the app is running". Views now read `crate::theme::theme(cx)` inside
  `render`; there is no `theme` field left in `crates/app`, and
  `Workspace::new`/`Pane::new`/the other eleven constructors lost the
  parameter.
* **The built-ins are JSON** (`crates/theme/themes/graphite-{dark,light}.json`,
  embedded at compile time) written in `hsl()` rather than hex — which is
  what makes them **bit-identical** to the palettes `theme.rs` carried
  through M6, and is why this refactor moved no visual baseline. Two tests
  assert that literal by literal.
* **User themes hot-reload.** `~/Library/Application Support/file-explorer/themes/*.json`
  is watched with the same `Vfs::watch` the pane uses for the open directory;
  a save re-reads the folder on the background executor and repaints. A
  partial theme inherits the built-in of its `appearance`; a broken one is
  reported and skipped, leaving the last good copy painted; a deleted one
  falls back to the default **without forgetting the user's choice**.
* **`settings.json` gained `theme`** (`Option<String>`), applied when the
  settings load lands. The picker that writes it, the settings window and
  keymap overrides are **M7b**, still to come.
...
- **M7a: `appearance: system` is not implemented.** Plan §6 wants a theme
  that follows macOS and switches live on the system appearance
  notification; the model has `light` and `dark` only, and the app never
  observes `WindowAppearance`. Everything it needs is now in place (one
  global, one `select`), so it is a contained addition. *M7b.*
