...
**M7b complete in code** — the settings store, system appearance and keymap
overrides, on top of M7a's theme system. Everything in this lane is
**invisible**: no baseline moves, because both new behaviors default to what
the app already did.

* **A real settings store.** `settings/defaults.json` is compiled in and
  complete; the file on disk is a partial overlay, so a missing or unknown key
  can never leave a setting undefined (unknown keys are *reported*, not
  dropped). The file is watched — editing it in a text editor applies live —
  writes are **serialized** through a single-slot writer so two quick changes
  cannot race two `atomic_write`s at one path, and only keys that differ from
  the defaults are written.
* **The §3 behaviors that are settings are wired**: `folders_first` (on by
  default) feeds every pane's `SortSpec` and fans out on change like the
  hidden-files toggle; `confirm_delete_to_trash` (off by default, because the
  trash is undoable) turns the bare `delete` key into a confirmation, routed
  up as `DirViewEvent::ConfirmTrash` → `PaneEvent::ConfirmTrash` → the
  workspace's existing dialog.
* **`appearance: system`.** `ThemeSelection` is either one theme or a
  light/dark pair; `Workspace` observes `observe_window_appearance` and the
  theme re-resolves through one `resolve()`. A static selection ignores the
  flip entirely.
* **Keymap overrides.** `keymap.json` layers over the §0 defaults — bare or
  qualified action names, `[name, data]`, `null` to unbind — applied by
  rebuilding the keymap, so deleting a row restores the default. Every bad row
  is reported and skipped: a typo must never leave the app without a keyboard.
* **One real bug caught by writing it down**: a settings load used to push its
  theme unconditionally, which would have overridden the visual-test runner's
  per-scenario theme and captured every light scenario in the dark one. A load
  now pushes the theme only if the file changed it.

The settings **window**, the theme picker, and the polish that paints
`selection`/`file_colors` are **M7c**, and that is the milestone's one
...
- **M7b: the settings file's `warnings` and the keymap's `diagnostics` are
  collected and never shown.** Both are exactly what the settings window will
  display, and until it exists a typo in either file is silent from the
  user's point of view (the behavior is correct — the key is ignored, the row
  is skipped — it is only unannounced). *M7c.*
- **M7b: nothing writes the settings file from the UI yet.** Every setting is
  readable, wired and hot-reloadable, but the only way to change one is to
  edit `settings.json`. *M7c.*
...
- **M7a: `selection` and `file_colors` are in the model but nothing paints
  them.** Selected rows still draw `accent.opacity(0.30)` and file glyphs are
  untinted, exactly as through M6. Adopting them is a visible change that
  moves every baseline, so it is held for the M7b polish pass where the
  baselines are regenerated once. Until then a theme can set them and see
  nothing happen.
- **M7a: no UI reaches the theme system yet.** There is no picker, no
  "reveal themes folder", and `ActiveTheme::diagnostics` (the per-file
  complaints from the last folder read) is collected and never shown — a
  user theme with a typo in it fails silently from the user's point of view.
  All three are the settings window's, *M7b*.
