...
**M7d-b done** — the sidebar is a peer of Finder's, and M7d is complete.

* **Five sections, one order**: Devices, Locations, Favorites, Recents, Tags.
* **Two levels, always.** M2's Explorer-style folder *tree* is deleted. It
  could expand to arbitrary depth (`Macintosh HD › dev › fd › 3 › …`), which
  made the sidebar a second, worse file browser beside the real one. Depth is
  navigated in the file pane; nothing in the sidebar expands.
* **Collapsing reflows.** One scrolling column, every section an ordinary
  block, nothing `flex_1` — so collapsing a section moves everything below it
  up. This closes M7c's "the sidebar cannot scroll as a whole, and Tags is now
  last" gap, and it closes it *without* the heterogeneous-`uniform_list`
  refactor that gap was expected to need: deleting the tree deleted the only
  unbounded content in the column.
* **Section chevrons moved to the far right**, where Finder and ForkLift put
  them, so the section titles share one left margin.
* **Locations** resolves on this machine — iCloud Drive, every OneDrive tenant
  root, home, `/Network`, Trash — and lists only what exists.
  **`/Network` turned out to be a real path** (an autofs mount), contrary to
  the plan's note that it "is not a path at all", so it needed no platform
  work. **AirDrop is deliberately absent**: it has no filesystem presence to
  navigate to.
* **Favorites are seeded once** with Desktop/Documents/Downloads, guarded by a
  flag rather than by emptiness, so unpinning them all stays unpinned.
* **Recents** tracks the folders the panes open, workspace-level so a split
  workspace's two panes feed one list.
* **A settings-destroying bug, found by running the app and fixed.** Both of
  this milestone's new writers fire during startup, and both used to run
  *before* `settings::init_with_path`'s background load landed. That loader
  **discards its result** when the content changed meanwhile (the
  "don't clobber a deliberate change" guard) — so an early write does not
  merely race the load: the user's entire file is thrown away and the
  defaults are persisted over it. On a real profile it silently deleted a
  saved `Graphite Light` and a pinned favorite. `AppSettings::is_loaded()`
  now gates both writers.
* **Row icons everywhere**, and the details list gained a type icon per row —
  M7d-a replaced the *control* glyphs but never added row icons, because there
  were none there to replace.

M7d-a remains as built below — the icon set.

**M7d-a done** — the app has a real icon set. Every control that was a
Unicode character in a string is now a vendored Lucide SVG, tinted from the
theme: `▸ ⏏ ⌕ ✕ ▣ ▢ ⓘ ⚙ ☰ ▦ ☐ ☑ ▲ ▼` are all gone.

* **Fifteen ISC-licensed SVGs**, `include_bytes!`-compiled and painted through
  `gpui::svg().data(..)` rather than a `gpui::AssetSource` — an asset source
  is installed on the `Application`, which no `#[gpui::test]` and no
  visual-runner frame ever builds, so path-addressed icons would be present in
  the shipped app and silently blank in every baseline. It also leaves
  `Cargo.toml` alone, so no full workspace rebuild.
* **Colour is never implicit.** gpui tints an SVG from the element's *own*
  `text_color` and does not inherit the ambient text style the way a string
  child does, so `icons::icon`/`sized_icon` take an `Hsla` and the two
  hover-recolouring glyphs (favorites unpin, search clear) moved their
  `.hover()` onto the `svg` itself.
* **Two glyphs the M7d brief did not name went with them**, because they are
  the same problem: the details view's sort caret (a filled `▲`/`▼` beside
  chevron disclosures) and the search field's "Subfolders", which was the
  characters `☐`/`☑` and so read visibly lighter and smaller than the
  checkbox two panels over. Fixing the second collapsed three independent
  checkbox drawings into one `icons::check_box`.
* **All 23 baselines move** and were rendered and *looked at* locally before
  the runner regenerates them — the icon set's whole failure mode is painting
  nothing at all, which a pixel diff reports identically to any other change.

**M7d-b still owes the rest of the milestone**: seeded default Favorites, a
**Locations** section, and making the sidebar one contiguous scroll region.

M7c remains as built below — the settings pane, on top of M7a (the theme
system) and M7b (the store, system appearance, keymap overrides).

* **A settings pane, not a window.** `cmd-,` replaces the browsing region
  with it; the sidebar and titlebar stay, so the app is visibly itself
  underneath and a theme change can be watched landing. Three sections:
  *General* (the two §3 behaviors), *Appearance* (follow-the-system plus every
  installed theme), *Keyboard* (every binding in force, read-only).
* **No OK/Cancel** — a click applies and persists immediately, which is what
  M7b's serialized writer exists to make safe.
* **The M7b diagnostics are on screen at last**, beneath their section, and
  they state the all-clear explicitly rather than showing nothing.
* **`selection` is painted.** The details list and icon grid take the
  selected-row tint from the theme token rather than deriving it from the
  accent; the built-ins define it as exactly what they painted before, so
  nothing moved.
* **The macOS menu bar exists, and it is a bug fix, not a flourish.** Every
  `cmd-` chord in the §0 table had been dead in the real app since M1: macOS
  offers Command chords to the main menu before the window sees them, and we
  had no menu, so the OS ate all of them. Unmodified keys always worked,
  which is why five milestones of mouse-driven demos never noticed.
  `cmd-q` is new with it — the app could not previously be quit from the
  keyboard. This is M8's menu-bar row, pulled forward.
* **The titlebar is ours.** macOS was drawing a grey bar above the workspace's
  themed row, so a theme that re-tinted the frame visibly stopped at the top
  of the window; `appears_transparent` puts the traffic lights inside our row.
  The app also calls itself "File Explorer" now, clear of those lights.

**A test-suite blind spot, now documented**: gpui's test platform dispatches
keystrokes straight into the keymap and never involves AppKit, so the §9
dispatch guards pass happily on chords the OS never delivers. The standing
guard is `menus::tests::every_command_chord_in_the_keymap_has_a_menu_item`,
...
- **M7d-c: Recents can lose the last few seconds to a hard kill.** The write
  is debounced by `RECENTS_FLUSH_DELAY` (3s), so a crash or force-quit inside
  that window drops the folders visited in it. Deliberate: Recents is a
  convenience, the folders are still there, and the alternative was a full
  rewrite of `settings.json` on every click.
- **M7d-b: the folder open at boot is not recorded in Recents.** Both startup
  writers gate on `AppSettings::is_loaded()`, and `main`'s boot navigation to
  home happens inside that window — so the first launch shows an empty Recents
  until you navigate somewhere. Correct (the alternative destroyed the user's
  settings file) but slightly short: flushing the active pane's current path
  once the load resolves would close it, and needs the workspace to observe
  `AppSettings`.
- **M7d-b: Trash is the boot volume's only.** `~/.Trash` does not union in
  `/Volumes/<name>/.Trashes/<uid>` the way Finder's single Trash icon does, so
  a file trashed from an external drive will not appear there. Pinned by
  `trash_is_the_boot_volumes_only`.
...
- **M7b: three actions cannot be bound from `keymap.json`.** `Paste`,
  `SortBy` and `ToggleTag` carry parameters and are declared `no_json`
  (avoiding a `schemars` dependency, per the note in `actions.rs` since M1),
  so the override file cannot build them. The loader says so in as many
  words — "`file_explorer::Paste` exists but cannot be bound from keymap.json
  (it takes parameters)" — rather than the lie that no such action exists.
  Two of the three are documented as mouse-only anyway; closing this means a
  `Cargo.toml` change, which forces a full workspace rebuild, so it was not
  done mid-lane.
- **M7c: menu items do not follow a `keymap.json` override after a reload.**
  gpui reads each item's key equivalent off the keymap when the menu is
  built, and `menus::init` runs once at boot — so a chord rebound in
  `keymap.json` later shows the old equivalent beside the menu item, and
  (because macOS resolves Command chords through the menu) the *old* chord
  keeps working while the new one does not. Rebuilding the menu on every
  keymap reload is the fix.
- **M7c: the menu bar is minimal.** It covers every action that has a
...
- **M7c: the Keyboard section is read-only.** It lists what the keymap
  ...
  the user may also be hand-editing, which is its own piece of work.
- **M7c: the Keyboard list is long, unfiltered and ungrouped.** ~70 rows
  sorted by context, with no headers and no search. Fine for "what does f2
  do"; poor for browsing.
- **M7c: `file_colors` is still modelled but unpainted, and there are still
  only two entry icons.** Wiring `selection` was a token swap with an
  identical result; per-kind icon tints remain a design step. M7d-a moved half
  of it: the folder/file shapes are real artwork now rather than `▣`/`▢`, and
  `icons::entry_icon` is the one seam a kind→icon classification would land
  behind — but it still answers "folder" or "not a folder" and nothing else,
  so an archive, a movie and a spreadsheet share one page glyph.
- **M7d-a: nothing automatically proves an icon rasterizes to visible
  pixels.** The unit tests walk `Icon::ALL` and check the vendored *bytes*
  (present, non-empty, 24×24, `currentColor`); whether gpui then paints them
  is only visible in the baselines. That is a real gap because the failure is
  silent and symmetric — an icon that renders as nothing produces a diff
  indistinguishable from an icon that renders correctly-but-changed, which is
  exactly the M4 trap CLAUDE.md's definition of done was written around. The
  standing mitigation is procedural, not automated: open the PNGs.
- **M7b: the system-appearance observer lives on the window.** With no window
  open — every headless test — a system flip is invisible; `ActiveTheme` reads
  the appearance once at `init` and then only hears from
  `Window::observe_window_appearance`. Correct for a single-window app, and
  the second window M8's tabs might bring would register a second observer
  reporting the same thing (harmless, since a repeat is a no-op).
- **M7c: there is no "reveal the themes folder" affordance.** The Appearance
  section tells you where to put a theme file; it cannot open that folder for
  you, which is a slightly absurd gap in a file manager.
- **M7a: the vendored text input holds its colors as state.** Placeholder,
  cursor and selection colors live on `InputState`, so a live theme switch
  reaches a text field only because its owner pushes them in from `render`
  (`refresh_input_colors`). A future widget that owns an `InputState` and
  forgets that call will paint the old theme's colors, and nothing catches
  it at compile time. The honest fix is for the vendored widget to read the
  global itself, which is a bigger vendored-code divergence than M7a wanted.
- **M7a: a theme file is re-read in full on every watcher batch.** The
  folder is tiny and the debounce is 150 ms, so this is a non-issue in
  practice; noted because the pane's listing watcher patches incrementally
...
| Date | PR | Change |
|---|---|---|
| 2026-09-11 | — | **M7d-c: the load is a merge, and startup writes nothing.** Three changes, from the question "why are we writing at startup at all?". **(1) The clobber guard is gone.** `init_with_path` used to ask one all-or-nothing question — "is the content still exactly the defaults?" — and, if not, **discard the loaded file**, which is how M7d-b destroyed a real profile's saved theme. `apply_loaded` now merges field by field: a field the app has changed since boot keeps the app's value, every other field takes the file's. Nothing is discarded and nothing in flight is clobbered. Tracked by `ChangedSinceBoot`, set by each mutator. The test that pinned the hazard now pins the merge instead — `a_write_inside_the_load_window_keeps_both_it_and_the_file` asserts *both* halves, since the interesting property is that the in-flight change **and** the untouched fields survive together. **(2) Seeding never writes.** The seed is deterministic — the same defaults filtered by the same existence check — so it is derived in memory on every launch instead of persisted, and reaches disk only when the user next changes something (`save` writes the whole document). "Emptying them stays empty" still holds: unpinning is a user change, so the empty list and the seeded flag persist together. A brand-new profile now boots to a fully populated sidebar having written **nothing** — verified on a real Mac by deleting `settings.json` and launching. **(3) Recents is debounced** (`RECENTS_FLUSH_DELAY`, 3s). Clicking through four folders was four full rewrites of `settings.json`, each a temp file plus a rename, for a list only interesting once the user stops moving; it is now one write once they settle. The sidebar still repaints immediately — the debounce delays the *disk*, never the UI, and the write was already off the UI thread. `FakeVfs::write_count` is new, because "this did **not** touch the disk" is not expressible with content or mtime assertions: a write producing identical bytes is still a write. 631 tests, 5 new; both write-reduction tests were verified to fail when their fix is removed (4 writes instead of 1; 1 instead of 0). |
| 2026-09-11 | — | **M7d-b fix: startup writes no longer destroy the settings file.** Found by running the app against a real profile rather than a fixture. `settings::init_with_path` seeds the global with defaults, loads the file in the background, and **declines to apply that load if the content changed meanwhile** — so a write inside that window makes the whole file be discarded and the defaults persisted over it. M7d-b added two writers that fire exactly there: the sidebar's favorites seeding, and the workspace's Recents (because `main` navigates to home immediately at boot). The result was a silently deleted `"theme": "Graphite Light"` and a lost favorite. `AppSettings::is_loaded()` is new and gates both; the sidebar's seeding is retried from the settings observer so whichever of the two startup conditions resolves last performs it — and reads `favorites_seeded()` before `update_global`, because `update_global` notifies the observer it is called from and the unconditional version recursed forever. 628 tests, 4 more: the hazard itself is pinned at the settings layer (`a_write_inside_the_load_window_discards_the_file`, which asserts the *unfixed* behavior honestly so nobody mistakes `is_loaded` for something the module enforces), and the gates are pinned at the call sites (`neither_startup_writer_touches_settings_before_the_load_lands`, verified to fail with either gate removed). |
| 2026-09-11 | — | **M7d-b: the sidebar rebuilt.** Five sections in one order — Devices, Locations, Favorites, Recents, Tags — and **two levels of hierarchy, full stop**. M2's Explorer-style folder tree is deleted (`TreeRow`, the `expanded` set, the per-node `children` cache, `_child_loads`, `flat`/`reflatten`/`flatten_into`, `toggle_expanded`, `invalidate_children`, and the `PaneEvent::DirsChanged` forward in `workspace.rs` that existed only to invalidate that cache). It could open to arbitrary depth, which made the sidebar a second, worse file browser beside the real one. **Collapsing a section now moves everything below it up**: one `overflow_y_scroll` column, every section an ordinary block, nothing `flex_1`. That closes M7c's scroll gap *without* the heterogeneous-`uniform_list` refactor it was expected to need — deleting the tree deleted the only unbounded content. **Section chevrons moved to the far right** (Finder and ForkLift both), so titles share one left margin. **Locations** is new: `fs-core/locations.rs` (GPUI-free, `Vfs`-driven) resolves iCloud Drive, every OneDrive tenant root, home, `/Network` and Trash, stats them, and returns only what exists — a row pointing at a missing folder navigates to an error. Two plan corrections fell out of checking the development Mac rather than assuming: **`/Network` is a real path** (autofs, `-> /System/Volumes/Data/Network`, containing `Servers`), so it needed none of the platform work the plan budgeted; and **AirDrop genuinely has no filesystem presence**, so it is omitted rather than faked. OneDrive is *matched* rather than fixed, because the sync root is tenant-suffixed (`OneDrive - BidOne Ltd`) — and matched as `OneDrive` or `OneDrive - <anything>`, not "starts with OneDrive", which would claim a user's own `OneDriveBackups`. **Favorites seed once** with Desktop/Documents/Downloads filtered to what exists, guarded by a `favorites_seeded` flag rather than by emptiness so unpinning them all stays unpinned; **Applications is excluded** because Finder's points at `/Applications` while a Mac also has `~/Applications` — one name, two folders. **Recents** is workspace-level (`Pane::load` emits `Navigated`, guarded on the same `path_changed` that tears down a rename so an in-place reload does not re-record; `Workspace` pushes onto `AppSettings`), capped at 10, deduplicated by moving to the front. **Row icons everywhere** — and the details list gained a type icon per row, which it had never had: the only thing separating a folder row from a file row was the disclosure chevron, and a folder among *search results* does not get one (M6a), so in search results they were indistinguishable. `FsContext` gained `home: PathBuf` so Locations and seeding resolve inside the fixture tree under test — with `env::home_dir()` inline they would look for `/Users/<someone>` in a `FakeVfs` holding only `/home` and render an empty Locations section into a plausible-looking baseline. 624 tests, 24 new (4 unit + 6 integration in `fs-core`, 8 settings, 8 sidebar UI — including `collapsing_a_section_moves_the_ones_below_it_up`, which asserts on painted geometry rather than on the section list). `sidebar_tree_expanded` is replaced by `sidebar_sections_collapsed`; all 24 baselines move. |
| 2026-09-11 | — | **M7d-a: the icon set.** `icons.rs` + `assets/icons/` — fifteen vendored Lucide SVGs (ISC, unmodified, recorded in `VENDORED.md`), one per `Icon` variant, `include_bytes!`-compiled and painted with `gpui::svg().data(..)`. **Deliberately not a `gpui::AssetSource`**: an asset source is installed on the `Application`, which no `#[gpui::test]` context and no visual-runner frame ever constructs, so path-addressed icons would work in the shipped app and be silently absent from every baseline we regenerate from — the exact shape of the M4 "every filename rendered as nothing" miss. Bytes in the binary are the same bytes everywhere, and it keeps `Cargo.toml` untouched (no `rust-embed`), so no forced full rebuild. Replaced in ten files: the sidebar's section/tree disclosures, eject, favorites `+` and unpin; the details list's disclosure **and its sort caret**; the icon grid's and info panel's folder/file placeholders; the titlebar's split/info/settings buttons; the pane's view-mode switcher; the search field's magnifier, clear and "Subfolders"; the jobs popover's cancel; the context menu's submenu chevron and tick; the settings pane's tick and filter magnifier. `theme::DISCLOSURE_*` is deleted in favour of `icons::disclosure(expanded)`. **Colour is explicit everywhere** because gpui tints an SVG from the element's *own* `text_color` and, unlike a string child, does not inherit the ambient text style — so the favorites unpin and the search clear now carry `.hover()` on the `svg` rather than on the wrapper, where it would have been a silent no-op. Two glyphs beyond the M7d brief went too, being the same defect: the `▲`/`▼` sort caret (a filled triangle beside chevron disclosures), and "Subfolders", which was `☐`/`☑` characters and so rendered lighter and smaller than a real checkbox — closing it collapsed **three** independently-drawn checkboxes (settings, info panel, search) into one `icons::check_box`, with colours as parameters so the info panel can still dim its unloaded rows to `DISABLED_ALPHA`. 600 tests, 4 new: `Icon::ALL` is walked to assert every file is present, non-empty, on the 24×24 grid and stroked in `currentColor`, and that no two variants share artwork (a `Close` drawing a `Plus` is a control, just the wrong one, and nothing downstream would fail). All 23 baselines move; every one was rendered locally and opened before acceptance, since "painted nothing" and "painted correctly" are the same pixel diff. |
| 2026-09-10 | — | **M7c: the Keyboard filter, chevrons, and Tags last.** The Keyboard section grew a filter field in its header (the vendored input, wired through the shared `rename::with_editor_actions` so it gets the same dispatch node — and the same `track_focus` of the input's own handle — every other text field has). It matches against the chord, the action **and** the context, because all three are things a person types ("cmd", "paste", "DirView"), terms are AND-ed so a second word narrows, and a count beneath reads "12 of 71" — or names the query when nothing matches, so an empty result reads as an empty *result* rather than an empty keymap. `escape` clears the field rather than closing the pane, the field being the innermost focused thing. A unit test caught a trap in the matcher: it lowercased the list but relied on its *caller* to lowercase the query, which worked only by luck of the single call site; it normalizes its own input now. **Disclosure chevrons** (`›`/`⌄`) replace the filled triangles in all three places that draw one — the sidebar's section headers, the sidebar tree and the details list — now sharing one pair of constants in `theme.rs` so they cannot drift; triangles read as a heavier, more structural control than a disclosure wants to be, and both Finder and ForkLift use chevrons. **Tags moved below Folders**, where Finder and ForkLift both put them: the tree above it is the unbounded, scrollable thing, and a fixed-length list of colours sitting between Favorites and the tree pushed the tree down for no reason. Because the tree is `flex_1`, Tags now sits as a footer pinned to the sidebar's bottom rather than contiguous with the tree as in Finder — left that way deliberately, since M7d rebuilds the sidebar's sections (Locations, seeded Favorites) and that is the moment to make the whole sidebar one scroll region instead of doing it twice. 596 tests, 2 new (the filter narrowing the list through the real field, and the AND-ing/case rules as a unit). |
| 2026-09-10 | — | **M7c: the macOS menu bar — a bug fix, not a flourish** (`crates/app/src/menus.rs`, new). Trying the new `cmd-,` in the real app turned up something much larger: **every `cmd-` chord in the §0 table had been dead since M1**. macOS offers Command chords to the main menu before anything else sees them, and an app with no main menu never gets them back — so `cmd-l`, `cmd-f`, `cmd-z`, `cmd-a`, `cmd-x/c/v`, `cmd-1/2`, `cmd-r`, `cmd-[`/`]` and `cmd-shift-n/o/i/.` all silently did nothing, while every unmodified key (Enter, F2, Delete, arrows, type-ahead) worked. Five milestones of mouse-driven demos never tripped over it. **Diagnosis was empirical, after two wrong theories** — the `DummyKeyboardMapper` in `KeyBinding::new` (a red herring: with `use_key_equivalents: false` the real macOS mapper behaves identically) and "modifiers are being stripped" (they were not). What settled it was `FE_LOG_KEYS=1`, a **capture-phase** key logger on the workspace root: a chord consumed by a binding never bubbles, so a bubble-phase listener cannot distinguish "the OS never delivered it" from "a binding handled it", and the first version of the logger could not tell those apart. With it, pressing `a`, `cmd-a`, `cmd-,` produced exactly one line — the bare `a`. The fix is a real menu bar (File Explorer / File / Edit / View / Go), every item dispatching the **same boxed action** the keymap and context menus do, so it is another trigger rather than a second code path; this is ARCHITECTURE.md's M8 menu-bar row pulled forward, because the keyboard does not work without it. **The six editing commands are declared with their `OsAction`**, which is load-bearing: a plain menu item binds a chord to one fixed action whatever has focus, so `cmd-c` in the rename editor or address bar would have copied the selected *files* instead of the selected text; the OS action wires them to AppKit's standard selectors so a focused text field handles them natively. `Quit`/`cmd-q` is new with it — the app could not previously be quit from the keyboard at all. **The standing guard** is `every_command_chord_in_the_keymap_has_a_menu_item`, which reads the **live keymap** and fails when a platform-modifier chord has no menu item, so a future `cmd-` binding added without one is caught at test time instead of silently not working; `visible_bindings` now returns a `BindingRow` whose `uses_platform_modifier` asks the modifier flags rather than the rendered text (`⌘` on macOS, `⊞`/`❖` elsewhere), which also fixed the settings pane's Keyboard list showing proper Mac symbols. **A blind spot worth naming**: gpui's test platform pushes keystrokes straight into the keymap and never involves AppKit, so §9's dispatch guards pass on chords the OS never delivers — this class of bug is structurally invisible to the suite, and the only instrument for key *delivery* is the running app. 594 tests (195 fs-core unit + 21 fs-core integration + 355 app + 23 theme), 3 new here: the two menu guards, and a `cmd-,` dispatch test with focus in the **file list** rather than on the workspace root — where focus actually sits when a user reaches for the chord, and the only place §9's silent-`track_focus` hazard would show. **Gaps opened:** the menu is built once at boot, so a `keymap.json` override applied later leaves the stale chord beside the item (and, since macOS resolves through the menu, the *old* chord keeps working); and the menu is minimal — actions without a chord, and the Window/Help menus, are M8's. |
| 2026-09-09 | — | **M7c: the settings pane** (`crates/app/src/settings_ui.rs`, new), plus the titlebar work folded in because it moves baselines and this PR regenerates them once. **A pane, not a window and not a modal** — `cmd-,` replaces the browsing region (pane strip *and* info panel) while the sidebar and titlebar stay put, `escape` closes it and hands focus back to the file list. The modal option was rejected on a functional ground rather than a stylistic one: it would block the window, and a settings surface whose whole job is changing how the window looks is only useful if the change can be watched landing behind it. Three sections. *General* carries the two §3 behaviors, each row with a line of why it defaults as it does ("The Trash is undoable, so this is off by default — Explorer does not ask"). *Appearance* is the picker: "Follow the system" plus every theme in the registry, drawn with a radio dot rather than a checkbox because they are one-of-many, and the shape should say so before the label is read. *Keyboard* lists every binding in force — **read back out of gpui** via the new `keymap::visible_bindings` rather than re-derived from the §0 table, so a `keymap.json` override appears as the override and a row the file failed to bind is visibly *absent* rather than listed as though it worked (`NoAction` unbinds are dropped). **No OK/Cancel**: a click mutates `AppSettings` and saves immediately, matching favorites since M2 and safe because M7b serializes the writes; a theme click applies to the running app *before* it persists, so the repaint never waits on a disk write. **Two observers, and the second is load-bearing** — a test caught it: `AppSettings` covers this pane's writes and a text editor's, but a user theme dropped into the themes folder joins the registry *without changing the painted theme*, so `ActiveTheme` never refreshes the windows and the picker went on listing the themes that existed when it opened; it observes `ActiveTheme` too now. **Both M7b diagnostic channels finally render**, beneath their section, stating the all-clear in words because an empty error area and a missing error area look identical. **`selection` is painted**: the details list and icon grid take the selected-row tint from `theme.selection` instead of deriving it from the accent, so a theme can re-tint selection without dragging every focus ring and hover state along — and the built-ins define `selection` as *exactly* what those views painted before (accent at 0.35 alpha, pinned by a theme-crate test), so that half moved nothing. `SELECTION_ALPHA` is gone from both views. **Titlebar** (folded in): macOS was drawing its own grey bar above the workspace's themed row, so the window had two bars and a theme that re-tinted the frame visibly stopped at the top of it — `appears_transparent` hides the system bar and keeps the traffic lights, which now sit inside the 40px row at 14/14, and the row's padding became an asymmetric `pl(100)`/`pr(80)` so the app name clears them; the name is the product's ("File Explorer"), not the crate's, and is also the macOS window title. The transparency half is `main.rs` only and moves nothing; the name and inset move all 21 baselines. Two new scenarios, `settings_general` and `settings_appearance` — the two sections with controls whose *state* can be wrong; Keyboard's interesting property is that it reflects the live keymap, which a unit test pins better than pixels can. 591 tests (195 fs-core unit + 21 fs-core integration + 352 app + 23 theme), 11 new (10 for the pane: open/close, tab switching, a behavior toggle reaching both the store and the disk *and* removing the override again when set back, the trash toggle, a theme applying and persisting, the follow-the-system pair being written as a pair, a hot-reloaded theme appearing in the picker, a broken theme file reported in the pane, the keymap list reflecting an override, and a user theme tinting selection without moving the accent; and 1 theme-crate pin for the built-in selection alpha, which is what makes the "nothing moved" claim checkable). **Gaps:** the Keyboard section is read-only and its list is long, unfiltered and ungrouped; `file_colors` is still unpainted (a token swap it is not — it needs a kind→colour classification and a look at the glyphs, so it was deferred rather than done badly); and there is no "reveal the themes folder" button, which is a slightly absurd thing for a file manager to lack. **Baselines still to regenerate** — the one runner run this milestone was split to require. |
| 2026-09-09 | — | **M7b: the settings store, system appearance and keymap overrides** — everything the settings window will sit on, none of it visible. `settings.rs` stops being the M2 stub and takes the shape M7a gave the theme system: `settings/defaults.json` is compiled in and **complete**, the file on disk deserializes into an all-optional partial and is overlaid on top, so no missing or unrecognised key can leave a setting undefined, and unknown keys land in `warnings()` instead of vanishing (a structurally broken file loads the defaults *and says so*, rather than looking like an empty one). The file is **watched** — 150 ms debounce, on the parent directory because an atomic write replaces the inode — so editing `settings.json` in a text editor applies live, and our own writes come back through it and are recognised as no-ops by comparison. Writes are **serialized**: `save()` parks the newest content in a single-slot `WriterState` and one writer task drains it, because two settings changed in quick succession were otherwise two `atomic_write`s racing at one path, where the second could land first and lose the newer content. Only keys that **differ from the defaults** are written, so the file stays small and a default changed later still reaches everyone who never overrode it — which broke four sidebar tests that had been parsing the written file as a whole `SettingsContent`, now reading it as what it is. **Both §3 behaviors are wired, and both default to exactly what the app already did — which is why this lane moves no baseline.** `folders_first` (on) feeds each pane's `SortSpec`, read at construction and fanned out by a new `Workspace::observe_global::<AppSettings>` handler on any change, the `ToggleHiddenFiles` shape; `confirm_delete_to_trash` (off — the trash is undoable, so Explorer does not ask) turns the bare `delete` key into a confirmation whose paths travel up as `DirViewEvent::ConfirmTrash` → `PaneEvent::ConfirmTrash` to the workspace's existing `ConfirmRequest` machinery, queueing nothing until it is confirmed. That made `handle_pane_event` a `subscribe_in`, since a dialog needs the window. **`appearance: system`** is `ThemeSelection::{Static, Dynamic{light,dark}}` in `crates/theme` (serde-`untagged`, so both spellings read naturally in `settings.json` and both round-trip) plus one `resolve()` in `ActiveTheme` that turns selection + system appearance + registry into the painted theme, driven by `Window::observe_window_appearance`; a static selection makes the callback a no-op. **`keymap.json`** layers over the §0 defaults with Zed's spelling — bare or fully-qualified action names, `[name, data]`, `null` to unbind via gpui's `NoAction` — applied by **rebuilding** (`clear_key_bindings`, defaults, then the file), so deleting a row restores the default with no accumulation. Nothing in that file is fatal: an unknown action, an unparseable keystroke and an invalid context expression are each reported into `UserKeymap::diagnostics` and skipped, because a typo must never leave the app without a keyboard. Bare names resolve when unambiguous, and `Paste` — ours *and* the vendored input's — is the case that proves the ambiguity message earns its keep. **One real bug, found by writing the runner interaction down**: `apply_loaded` pushed the loaded theme unconditionally, so the boot settings load would have overridden the visual runner's per-scenario theme and captured every light scenario in the dark one; a load now pushes the theme only if the file actually changed it, pinned by `a_settings_load_that_changes_no_theme_does_not_touch_the_theme`. 580 tests (195 fs-core unit + 21 fs-core integration + 342 app + 22 theme), 27 new: 7 settings (defaults, partial overlay, unknown/malformed reporting, the override-only write, serialized saves, the live external edit, the theme no-push), 10 keymap (override/qualified/bare/null/rebuild, all four failure modes, the ambiguity message, hot reload), 4 workspace (`folders_first` fan-out, and the trash confirmation off/on/cancelled), 3 theme-global (dynamic follows, static ignores, a dynamic pair still hot-reloads), 3 in the theme crate (both `ThemeSelection` forms and their JSON round trip). **Gaps opened:** `Paste`/`SortBy`/`ToggleTag` cannot be bound from `keymap.json` (they are `no_json` to avoid a `schemars` dependency; the loader explains rather than claiming they do not exist), and both diagnostic channels are collected but unshown until the window exists. **Gap closed:** `appearance: system`. |
| 2026-09-09 | — | **M7a: the theme system** (`crates/theme`, new + `crates/app/src/theme.rs`, rewritten). The layout has claimed a `crates/theme` since M0; it exists now, holding the model and the JSON loader and nothing else — gpui only for `Hsla`/`SharedString`, no I/O, so it builds and tests headless like `fs-core`. **The refactor is the milestone, not the file format.** Before this, every view took a `Theme` at construction and kept a clone, which cannot answer "the theme changed while the app is running"; `ActiveTheme` is now a gpui `Global` and views read `crate::theme::theme(cx)` inside `render`, so a switch is one `select` plus a window refresh. Thirteen constructors lost their `theme` parameter and every `theme` field in `crates/app` is gone. **This moved no pixels, and that is proven rather than hoped:** the two built-ins moved out of Rust literals into `themes/graphite-{dark,light}.json`, written in `hsl(240, 4%, 12%)` form precisely because `240.0/360.0` and `4%/100` reproduce the source literals bit for bit where hex would not (`hsl_parses_to_the_exact_same_bits_a_rust_literal_would_produce`, plus a per-field assertion of the whole pre-M7 palette for each appearance) — **so no baseline changed and no runner run is needed for this PR**. `Theme` `Deref`s to its `ThemeColors`, which is what kept ~50 `theme.surface`-style call sites untouched. **Partial user themes inherit** the built-in of their declared `appearance` — a two-line file that re-tints the accent is legal — and that scheme is kept non-circular by parsing the built-ins through a *complete* path that requires every key. Unknown keys become `warnings` rather than silence (a `"colours"` typo is reported); a bad color names the key carrying it. Colors parse as `hsl()`/`hsla()` **and** as `#rgb`/`#rrggbb`/`#rrggbbaa`, so a Zed/gpui-component theme can be adapted by copying its colors across. **Hot reload** watches `~/Library/Application Support/file-explorer/themes/` through the same `Vfs::watch` the pane uses for its open directory (150 ms debounce, `*.json` only, filename order); a batch re-reads the folder on the background executor and installs built-ins + everything that parsed. A broken file is recorded in `diagnostics` and **skipped** — the last good copy stays on screen — and a deleted theme falls back to the default while the *requested* name is remembered, so restoring the file restores the choice. Nothing here touches the disk on the UI thread: `BackgroundWatchGuard` was lifted out of `pane.rs` into `watch_guard.rs` and is now shared by both watchers. `settings.json` gained `theme: Option<String>`, applied when the settings load lands (a pre-M7 file still loads); the picker that writes it is M7b. **One widget genuinely holds painted colors as state** — the vendored `TextInput` — so `input::refresh_input_colors` pushes the current theme in from the address bar's, the search field's and the rename editor's `render`, with `InputState::set_colors` added as a local modification and deliberately not notifying. `FakeVfs::write_file` is new in fs-core: `insert_file` sizes a file without filling it and `insert_tree` fills one without announcing it, and a hot-reload test needs both at once. 553 tests (195 fs-core unit + 21 fs-core integration + 318 app + 19 theme), 30 new here: the whole theme crate (7 color — both spellings, round-trip, rejection with the offending text in the message — and 12 model/registry, including the two bit-identity pins), 8 `#[gpui::test]`s over the global (registry contents, install, select-is-remembered, edit-repaints, broken-file-skipped, delete-falls-back-and-restores, non-JSON ignored, and the watch being unregistered when the global goes away), 1 UI test driving a real window to prove a live switch reaches a focused text field, 1 `FakeVfs::write_file` test in fs-core, and 1 settings round-trip covering a pre-M7 file with no `theme` key. **Gaps opened:** `appearance: system` is not implemented; `selection`/`file_colors` are in the model but nothing paints them yet (deliberately — that is a baseline-moving change, held for M7b's one regeneration); no UI reaches the theme system, so `diagnostics` is collected and never shown. |
