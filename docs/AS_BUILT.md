...
- **M7d-b: the folder open at boot is not recorded in Recents.** Both startup
  ...
  `AppSettings`.
- **M7d-b: `settings::init_with_path`'s clobber guard is still sharp.** The
  fix gates the *callers*; the settings module itself will still discard a
  loaded file if anything mutated the global first, and nothing stops a future
  writer from reintroducing the bug.
  `settings::tests::a_write_inside_the_load_window_discards_the_file` pins the
  hazard so it is at least documented rather than latent. A merging load —
  apply the file, but keep fields changed since boot — would remove the trap
  entirely.
