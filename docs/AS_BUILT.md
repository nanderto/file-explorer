...
**M7c in progress** — the settings pane exists and M7's remaining gaps are
closing. On top of M7a (the theme system) and M7b (the store, system
appearance, keymap overrides).
...
- **M7c: `file_colors` is still modelled but unpainted.** Wiring `selection`
  was a token swap with an identical result; per-kind icon tints are a design
  step — a kind→colour classification plus a real look at the glyphs, which
  are currently one shape for folders and one for everything else. Deferred
