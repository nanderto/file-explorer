...
- **M6 — Search, tags, permission editing (2 wk).** Toolbar search filtering the current folder, then recursive with streamed results; Finder tags read/write + tag dots + sidebar tag filter; permission checkboxes and octal field actually chmod; owner/group change where privileged. *Accept: tagging a file here shows in Finder and vice versa.*
  Built as two PRs: **M6a — search** (both halves of the search sentence: the instant folder-local filter and the streamed recursive walk) and **M6b — tags + permission editing** (the rest). The split is delivery order only, not an architecture change; both are done. Two deviations landed in M6b, both recorded in `docs/AS_BUILT.md`: the info panel's Owner/Group are editable **name fields** rather than dropdowns (`Platform` can set an owner by name but cannot enumerate accounts), and legacy `com.apple.FinderInfo` label colours are not read. See `docs/AS_BUILT.md`.
- **M7 — Theme polish + settings (1 wk).** Settings window (general behaviors from §3 that are opt-out, keymap overrides, theme picker), custom-theme hot reload, second built-in theme. *Accept: a hand-written user theme applies without restart.*
  Split into three PRs, delivery order only (the split keeps the one
  baseline-moving change in a PR of its own): **M7a — the theme system** (the `crates/theme` model + JSON loader, the `ActiveTheme` global replacing per-view `Theme` clones, both built-ins as JSON, and the themes-folder hot reload — **done**; the acceptance criterion's "applies without restart" half is met and covered by `editing_a_theme_file_repaints_without_reselecting_it`) ; **M7b — the plumbing** (the real settings store: embedded defaults, watch, serialized atomic writes; the §3 opt-out behaviors `folders_first` and `confirm_delete_to_trash`, both defaulting to the existing behavior; `appearance: system` via `ThemeSelection` + `observe_window_appearance`; and `keymap.json` overrides — **done**, and visible nowhere, so no baseline moved); and **M7c — the settings pane** (the UI over all of the above — delivered as a *pane* that takes over the browsing region rather than a separate window, so a theme change can be watched landing behind it: theme picker, the behavior toggles, a read-only keymap list, both diagnostic channels, and `selection` painted from its own token; `file_colors` is deferred as a design step rather than a wiring one, and a rebinding UI with it — the one baseline regeneration of this milestone). See `docs/AS_BUILT.md`.
- **M7d — Sidebar and icons (new, carved out of M7c's review).** Split into two PRs, delivery order only — both halves move every visual baseline, and keeping them apart means each regeneration has one reviewable cause: **M7d-a — the icon set** (bundled ISC Lucide SVGs through `gpui::svg().data(..)`, replacing every Unicode glyph; **done** — see `docs/AS_BUILT.md`) and **M7d-b — the sidebar** (the rest of this entry; **done** — see `docs/AS_BUILT.md`).
  Three corrections to the scope below, all from checking the development Mac
  rather than assuming: **`/Network` is a real path** (an autofs mount), so it
  needed none of the platform work budgeted here; **AirDrop has no filesystem
  presence at all** and is omitted rather than faked; and the **folder tree was
  deleted** rather than made scrollable — the sidebar is two levels now, depth
  is navigated in the file pane, and removing the tree removed the only
  unbounded content, which is what made "one contiguous scroll region" fall
  out for free. A **Recents** section was added that this entry did not
  anticipate.
  Original scope: Seeded default Favorites (home, Desktop, Documents, Downloads, Applications, Pictures) with a "seeded once" flag, so emptying them stays empty; a **Locations** section as Finder has it (iCloud Drive and OneDrive are real paths to detect; AirDrop and Network are not paths at all and need platform work; Trash is special); and **a real icon set** — bundled MIT-licensed SVGs (Lucide/Feather) rendered through gpui's `svg()`, replacing the Unicode glyphs typed inline in the source that every "icon" in the app currently is (`▸ ⏏ ⌕ ✕ ▣ ▢ ⓘ ⚙`). SF Symbols were considered and rejected: the licence restricts redistribution, and glyph availability varies by macOS version, which would make the screenshot baselines fragile across runner images. Also the moment to make the sidebar one contiguous scroll region, since M7c left Tags pinned as a footer. *Accept: the sidebar reads as a peer of Finder's at a glance.*
