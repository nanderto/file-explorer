...
- **M7 — Theme polish + settings (1 wk).** Settings window (general behaviors from §3 that are opt-out, keymap overrides, theme picker), custom-theme hot reload, second built-in theme. *Accept: a hand-written user theme applies without restart.*
  Split into two PRs, delivery order only: **M7a — the theme system** (the `crates/theme` model + JSON loader, the `ActiveTheme` global replacing per-view `Theme` clones, both built-ins as JSON, and the themes-folder hot reload — **done**; the acceptance criterion's "applies without restart" half is met and covered by `editing_a_theme_file_repaints_without_reselecting_it`) and **M7b — the settings window** (the picker that writes `settings.json`'s `theme` key, the §3 opt-out behaviors, keymap overrides, `appearance: system`, and the polish that finally paints `selection`/`file_colors` — the one baseline regeneration of this milestone). See `docs/AS_BUILT.md`.
