...
- **M7d — Sidebar and icons (new, carved out of M7c's review).** Split into two PRs, delivery order only — both halves move every visual baseline, and keeping them apart means each regeneration has one reviewable cause: **M7d-a — the icon set** (bundled ISC Lucide SVGs through `gpui::svg().data(..)`, replacing every Unicode glyph; **done** — see `docs/AS_BUILT.md`) and **M7d-b — the sidebar** (the rest of this entry).
