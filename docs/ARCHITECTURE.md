...
| §2 Toolbar | Search field focus | `cmd-f` | `FocusSearch` | `Workspace` | Workspace → active Pane | M6a |
| §2 Toolbar | Toggle "Search subfolders" (folder-local ↔ recursive) | toggle click (mouse only) | *not an action* — a `SearchBar`-local toggle whose state the pane records, on the same precedent as the info panel's section headers: it is a control's own presentation state with no keymap row, menu item or other dispatcher. It becomes an action when M8's menu bar wants a **Search ▸ Include subfolders** item. | — | search.rs → Pane | M6a |
| §2 Toolbar | Clear the search, restore the unfiltered listing, give focus back to the pane | `escape` in the field | `Cancel` | `TextInput` (the search field's node) | SearchBar → `SearchBarEvent::Dismissed` → `Pane::cancel_search_for_navigation` | M6a |
| §2 Toolbar | Done typing: move focus to the results, query stays live | `enter` in the field | `Confirm` | `TextInput` (the search field's node) | SearchBar → `SearchBarEvent::Submitted` → Pane focuses the DirView | M6a |
| §2 Toolbar | Clear the search from the field itself | `✕` click (mouse only) | *not an action* — the same `SearchBarEvent::Dismissed` that `escape` emits, so the clearing logic exists once. | — | search.rs → Pane | M6a |
...
│       ├── tags.rs             # Tag, TagColor (macOS palette), _kMDItemUserTags codec  (M6b)
│       ├── ops/
...
│       └── platform/
│           ├── mod.rs          # Platform trait: volumes(), read/write/known_tags(), thumbnail(), open(), reveal()
...
└── app/                        # gpui. The only crate allowed to import gpui.
    ...
    └── src/
        ...
        ├── search.rs           # SearchBar widget + the Pane's SearchState  (M6a)
        ├── info_panel.rs       # (M5)
...
**`undo.rs`** — `UndoStack { undo: Vec<UndoEntry>, redo: Vec<UndoEntry> }`; `UndoEntry` = the *inverse* op recorded from `OpReceipt` at completion: rename→rename-back, move→move-back, copy→remove-created, new-folder→remove, trash→`Restore(TrashId)`. `apply` re-submits through the JobQueue (so undo gets progress/conflicts for free). Each entry stores expected `(path, mtime)` fingerprints; **if the world changed underneath, the entry is skipped with a user-visible toast** ("Can't undo — 'report.pdf' was modified since") rather than destroying data — the invalidation is a first-class, testable outcome (`UndoOutcome::Invalidated { entry, reason }`), not a silent drop. `Restore` failures map `TrashRestoreError` variants to their distinct toasts. **M6b's attribute ops need a different guard**: `chmod` changes ctime and *not* mtime, so an mtime fingerprint is structurally blind to them — the inverse is built from the previous values the job captured (`OpReceipt::restored_attrs: Vec<(PathBuf, PrevAttrs)>`, one inverse op per distinct previous value so a mixed selection is restored exactly) and guarded by `AttrGuard { path, expected: PrevAttrs }`, which re-reads the mode / owner+group / tag set the job wrote and invalidates on a mismatch.

**`platform/`** — `Platform` trait strictly for OS services, *not* file I/O: `volumes() -> Vec<VolumeInfo>`, `eject`, `read_tags`/`write_tags`/`known_tags` (M6b; the whole tag set is replaced per write, an empty set removes the xattr, and the payload's string form lives in the pure `tags.rs` codec), `set_ownership(path, owner, group)` (M6b — a `chown` *by name*, so the account lookup is the OS's; the mode is not here, it is `Vfs::mode`/`set_mode`, because a mode is file I/O), `thumbnail(path, px) -> Result<ImageBytes>`, `open(path)`, `open_with_menu(path)`, `reveal_in_finder`. `macos.rs` via objc2 (calls wrapped in `spawner.unblock`); `stub.rs` returns fixed fake volumes and no-ops so the whole workspace runs on this Windows machine.
...
| **Toolbar search field** (`search.rs`) | M6a | Hand-built around the vendored `TextInput`: `SearchBar` entity (magnifier glyph, `✕` clear, a mouse-only "Subfolders" toggle) at the right end of each pane's chrome row, emitting `SearchBarEvent` up to the pane. The pane holds `Option<SearchState>`; typing filters the snapshot through the pure `fs_core::filter_snapshot`, "Subfolders" runs `fs_core::search_recursive` on the background executor with its events folded in on a 100 ms `Spawner::timer` batch, in **one** cancellable `Task` slot. Results replace the `DirView`'s flat projection, so every gesture, hit test and observer downstream is unchanged — flat means `ProjectedRow::disclosure` is false and the expansion actions are inert, not just that nothing is indented. Accumulated hits are capped (`MAX_SEARCH_HITS`) because the per-batch dedupe+sort runs on the UI thread. The chrome row clips and both the breadcrumb and the field shrink, so a narrow split pane does not overflow. The node carrying `key_context("TextInput")` must also `track_focus` the input's own handle. |
...
| | `tags.rs` + `platform/` (M6b) | codec matrix (bare name, out-of-range index, non-numeric trailing line, blanks, duplicates, non-ASCII, empty array, order); `TagColor` indices pinned as a file format (a renumbering fails); palette rgba distinct and opaque; stub round-trip + empty-removes + `known_tags` = palette + user tags. **Finder compatibility** (`tests/tags.rs`, `cfg(macos)`): our write read back with `xattr -px` + `plutil -convert xml1`, a `plutil`-built binary plist installed with `xattr -wx` read by `read_tags`, an XML payload likewise, and (in `macos.rs`) Foundation's own `NSURLTagNamesKey` agreeing in both directions |
| | `ops/` + `undo.rs` attribute ops (M6b) | the shared `run_attrs` spine (previous value captured before each write, per-path progress); mode masked to `0o7777`; a denied/vanished path reported in `OpReceipt::failed` while the rest of the selection lands; an all-failed job `Failed` naming every reason; cancel between paths; a platformless queue failing only `Chown`/`SetTags`; `chmod` never touching mtime (FakeVfs *and* a real file); `RealVfs` chmod round-trip incl. setuid and symlink-follow; stub ownership as storage + `STUB_PRIVILEGED_OWNER` refused; macOS `set_ownership` refusing root/unknown accounts cleanly and detecting Foundation's silent no-op; mixed-selection undo restoring each path's own mode and redo re-applying; both invalidation reasons ("permissions changed since", "tags changed since"); guards excluding failed paths. **Integration** (`tests/attr_ops.rs`, real `tempfile` tree + `MacPlatform` on macOS): chmod → verify → undo → verify, tags → undo (real xattr), clearing tags undone, unprivileged chown failing with nothing changed |
| | `search.rs` (M6a) | typing filters the projection and reads **no** directory (recording `Vfs`, count zero across the keystroke); blank/whitespace restores it; the field drives the pane end-to-end; a filtered-out selection is dropped while expansion state survives; `escape` clears and returns focus; `cmd-f` focuses the *active* pane's field (in `workspace.rs`); recursive hits stream into the projection on fake time with the folder-local match not double-reported; rapid query changes leave exactly one walk (a `RecordingVfs` that parks before each `read_dir` — the superseded walk contributes one read and never resumes); navigation cancels the walk and empties the field; a watcher patch cannot unfilter; a refresh keeps the query; "Subfolders" clicked before the query text lands is still honored; a hidden-files toggle re-walks from scratch (the hidden hit leaves, nothing is double-counted); "Subfolders" off mid-walk stops reporting a running walk; emptying the field resets the toggle's mirror with it; a zero-result query says so instead of "Empty folder"; result rows carry no working disclosure and leave no expansion state behind; one batch rebuilds the rows once and the hit count is capped (thread-local `ROWS_REBUILT` probe) |
...
| **Visual scenarios** (`visual_test_runner`, macOS CI — infra exists per AS_BUILT) | per milestone | existing `workspace_dark/light`; add M1 `listing_populated`, `listing_sorted_by_size`, `address_bar_editing`; M2 `sidebar_tree_expanded`, `details_folder_expanded`; M3 `rename_editing`, `context_menu_open`, `conflict_dialog`, `cut_dimmed`, `marquee_active`; M4 `icon_grid`, `split_panes`; M5 `info_panel_jpeg` (this row's original name — a selected `.jpg`, the milestone's acceptance criterion) plus `info_panel_selection` (the same shape over a `.pdf` in a populated folder) and `info_panel_multi_selection` (the summary state, which nothing else pins); M6a `search_filtered` and `search_results` (the same query with "Subfolders" off and on, so the only difference between the two frames is what the recursive walk adds: the first pins the focused field, the clear button, the unchecked toggle, the instant folder-local filter and the count-only status line; the second pins the lit toggle, the finished "N folders searched" line and both kinds of result row — local matches unlabelled beside deeper hits carrying their containing-folder qualifier. `settle_search` advances the deterministic clock a throttle window at a time until the pane reports the walk finished, and hard-fails a search scenario that has no search, no result rows, the wrong scope, or (for the recursive one) no out-of-folder hit — so neither a partial result list, a plausible-looking empty pane, nor a folder-local frame under a lit checkbox can be baked into a baseline); M6b `info_panel_permissions` (a tagged file selected with the Permissions grid live and the octal editor open — the state a click on a field leaves the panel in, and one no other frame carries) and `tag_filter` (the sidebar's Tags section with a tag active, the rows the filter kept and the "N items tagged" status line; `settle_tag_filter` waits out the background scan exactly as `settle_search` does, and hard-fails on an empty row list); M7 `user_theme_applied`. All fed by FakeVfs fixture trees for determinism (fixed 1200×760, Helvetica, no wall-clock UI — per CLAUDE.md). |
...
**M6a — Search (built).** fs-core `search.rs`: `SearchQuery` + the pure `filter_snapshot` + the streamed breadth-first `search_recursive`. app `search.rs`: the `SearchBar` toolbar field (reusing `TextInput`, `FocusSearch`/`cmd-f` via the workspace) and the pane's `SearchState`, whose rows *are* the `DirView` projection. Tags and permission editing are M6b.
